...
function Add() {
  ...
  // Using useEffect to react to tagPlusVisible state change (When the user clicks on the plus sign in the tag area),
  ...
  // Setting up a side effect when the user has already clicked the "+" button in the tag area, then closes the tag panel then reopen it again (Improving UX by avoiding unnecesery double clicks)
  useEffect(() => {
    ...
    <>
      ...
      >
        ...
        <div className="img-container" onClick={() => divClick()}>
          ...
          />
          <label className="label" style={{ padding: "0.5rem" }}>
            {imageUploaded ? "Image successfully added!" : "Add Image"}
          </label>
        </div>
        {/* Title input */}
        ...
        <div className="tags-container">
          <div
            className="lineOne"
            onClick={
              tagsOpen ? () => setTagsOpen(false) : () => setTagsOpen(true)
            }
          >
            <div>
          ...
          {tagsOpen ? (
            <>
              ...
              <div className="lineTwo">
                {tagPlusVisible ? (
                  <span className="tag-input" onClick={() => addTag()}>
                    <FontAwesomeIcon
                      icon={faPlus}
                      className="icon"
                      style={{ fontSize: "1rem" }}
                  ...
                  <input
                    ref={tagInputRef}
                    value={tagInput}
                    onChange={(e) => tagInputonChange(e)}
                    className={
                      tagInput === tag ? "tag-input active" : "tag-input"
                    }
                    onClick={() => setTag(tagInput)}
                    style={{ width: `${tagInput.length}ch`, minWidth: "3rem" }}
                  />
                )}
              </div>
            </>
